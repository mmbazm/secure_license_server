...
class EnclaveOperations(enclaveOperations_pb2_grpc.EnclaveOperationsServicer):
    ...
    def get_last_enclave_id(self, session):
        ...
        """
        last_enclave = session.query(Enclave).order_by(Enclave.id.desc()).first()
    ...
    def EnclaveLaunch(self, request, context):
        ...
        try:
            ...
            access_token_entry = session.query(AccessToken).filter_by(username=request.username, access_token=request.access_token).first()
            ...
            res_enclaves = session.query(Enclave).filter_by(username=request.username).first()
            ...
            cert_helper.generate_server_certificate(self.server_root_cert_name, self.server_key_name, enclave_domain)

            # Generate hash of TLS certificate
            cert_hash = cert_helper.generate_hash_crt_file(f"{enclave_domain}.crt")
            cert_hash_signature = cert_helper.sign_hash_with_private_key(cert_hash, '../files/server_private_ak.pem')
    ...
    def EnclaveStatus(self, request, context):
        ...
        # Check if user has an enclave running
        session = Session()
        try:
            enclaves = session.query(Enclave).filter_by(username=request.username).all()
            if not enclaves:
...
def serve():
    ...
    enclaveOperations_pb2_grpc.add_EnclaveOperationsServicer_to_server(EnclaveOperations(), server)
